...
class BlueprintsServletContextListener extends ServletContextListener with ServletApiImplicits with Logging {
  private var lifeCycle: BlueprintsLifeCycle = _
  ...
  private def loadLifeCycle(): BlueprintsLifeCycle = Try {
    val bootstrapClass = Class.forName("Bootstrap")
    bootstrapClass.newInstance().asInstanceOf[BlueprintsLifeCycle]
  } getOrElse defaultBootstrap

  private lazy val defaultBootstrap = new BlueprintsLifeCycle {
